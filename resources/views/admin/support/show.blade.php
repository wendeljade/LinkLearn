            ...
            @if($ticket->status === 'open')
                <form action="{{ route('admin.support.reply', $ticket->id) }}" method="POST">
                    ...
                    <div style="position: relative;">
                        <textarea name="message" required class="form-input" style="width: 100%; padding: 1rem; padding-right: 6rem; border: 1px solid var(--border); border-radius: 1rem; min-height: 80px; resize: none;" placeholder="Type your response to the user..."></textarea>
                        <button type="submit" class="btn btn-primary" style="position: absolute; right: 0.5rem; bottom: 0.5rem; padding: 0.5rem 1rem; border-radius: 0.75rem;">Send Reply</button>
                    </div>
                </form>
            @else
